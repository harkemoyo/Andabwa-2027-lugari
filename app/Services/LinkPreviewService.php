...
class LinkPreviewService
{
    ...
    public function extract(string $url, bool $force = false): array
    {
        ...
        return Cache::remember($cacheKey, now()->addDay(), function () use ($url) {
            // 1. Resolve redirects first (bit.ly, t.co, etc)
            ...
            // 3. Other video platforms (Vimeo, Streamable, Direct MP4)
            if ($video = $this->detectVideoPlatform($finalUrl)) {
                return [
                    ...
                    'source' => parse_url($finalUrl, PHP_URL_HOST),
                ];
            }
     ...
     * Uses GET with a timeout because HEAD is often blocked by CDNs.
     */
    ...
    private function extractExternal(string $url): array
    {
        try {
            ...
            // Find Image (Meta -> First Img Tag -> Favicon)
    ...
    private function extractFirstImage(string $html, string $baseUrl): ?string
    {
        if (preg_match('/<img[^>]+src=["\']([^"\']+)["\'][^>]*>/i', $html, $matches)) {
            return $this->absUrl($matches[1], $baseUrl);
        }
        return null;
    }
