...
<div class="relative w-full aspect-video bg-slate-100 rounded-xl overflow-hidden group isolate">
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($featuredMediaUrl && !$isExternal): ?>
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($featuredMediaIsVideo): ?>
            <video 
                controls 
                preload="metadata"
                poster="<?php echo e($featuredMedia->hasGeneratedConversion('preview') ? $featuredMedia->getUrl('preview') : ''); ?>"
                class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
                <source src="<?php echo e($featuredMediaUrl); ?>" type="<?php echo e($featuredMedia->mime_type); ?>">
            </video>

            
            <div class="absolute top-3 left-3 bg-black/60 backdrop-blur-md text-white px-2.5 py-1 rounded-md text-xs font-medium z-10 flex items-center gap-1.5 shadow-sm">
    ...
    <?php elseif($isExternal && $mediaTypeValue === 'youtube'): ?>
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($imageUrl): ?>
            ...
            <div class="absolute inset-0 bg-gradient-to-br from-red-50 to-slate-100 hidden items-center justify-center p-6 z-0 text-center">
                <div>
                    ...
                    <p class="text-sm text-slate-600 font-medium line-clamp-2"><?php echo e($post->title); ?></p>
                </div>
            </div>
        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
        ...
        <button 
            onclick="document.getElementById('youtube-modal-<?php echo e($post->id); ?>').showModal()"
        ...
        <dialog id="youtube-modal-<?php echo e($post->id); ?>" class="fixed inset-0 m-auto max-w-5xl w-11/12 bg-transparent backdrop:bg-black/90 p-0 rounded-xl shadow-2xl z-50 overflow-hidden" onclick="if(event.target === this) this.close()">
            <div class="flex flex-col bg-slate-950 rounded-xl overflow-hidden shadow-2xl">
                <div class="flex items-center justify-between p-3 border-b border-slate-800">
                    <h2 class="text-sm font-medium text-slate-200 truncate pr-4"><?php echo e($post->title); ?></h2>
                    <form method="dialog">
    ...
    <?php elseif($isExternal && in_array($mediaTypeValue, ['article', 'external_link'])): ?>
        <?php $imageUrl = $data['image'] ?? null; ?>
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($imageUrl): ?>
            ...
            <div class="absolute inset-0 bg-gradient-to-br from-emerald-50 to-slate-100 hidden items-center justify-center p-6 z-0">
                <div class="text-center max-w-sm">
                    <div class="w-12 h-12 bg-white rounded-lg flex items-center justify-center mx-auto shadow-sm mb-3">
                    ...
                    <p class="text-sm font-semibold text-slate-900 line-clamp-2"><?php echo e($post->title); ?></p>
                </div>
            </div>
        <?php else: ?>
            ...
            <div class="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-emerald-50 to-slate-100 p-6 text-center">
                <div class="max-w-sm">
                    ...
                    <p class="text-sm font-semibold text-slate-800 line-clamp-2"><?php echo e($post->title); ?></p>
    ...
    <a href="<?php echo e(route('blog.external', $post->slug)); ?>"
       class="absolute top-3 right-3 z-20 px-2.5 py-1.5 text-xs font-semibold text-white rounded-md backdrop-blur-md shadow-sm transition-colors duration-200 pointer-events-auto
       <?php echo e($mediaTypeValue === 'youtube' ? 'bg-red-600/90 hover:bg-red-600' : 'bg-slate-900/70 hover:bg-slate-900'); ?>">
        
        <?php echo e($mediaTypeValue === 'youtube' ? 'Watch' : ($isExternal ? 'Visit Link' : 'View')); ?>

        
        <svg class="w-3 h-3 ml-1 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isExternal): ?>
...
</div><?php /**PATH C:\Users\Rygss\Downloads\andabwa-2027\resources\views/components/blog/media.blade.php ENDPATH**/ ?>