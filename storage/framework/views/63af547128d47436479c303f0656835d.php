...
<div class="relative w-full h-full overflow-hidden group bg-white">
    ...
    <?php elseif($isExternal && $mediaTypeValue === 'youtube'): ?>
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($imageUrl): ?>
            ...
            <div class="absolute inset-0 w-full h-full min-h-[224px] bg-gradient-to-br from-red-50 to-slate-100 flex items-center justify-center p-8 hidden" id="fallback-youtube-<?php echo e($post->id); ?>">
                <div class="text-center">
                    <svg class="w-16 h-16 mx-auto text-red-400 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
                    </svg>
                    <p class="text-slate-600 font-medium"><?php echo e($post->title); ?></p>
                </div>
            </div>
            
            <script>
                const youtubeImg = document.querySelector('img[src="<?php echo e($imageUrl); ?>"]');
                if (youtubeImg) {
                    youtubeImg.addEventListener('error', function() {
                        this.style.display = 'none';
                        const fallback = document.getElementById('fallback-youtube-<?php echo e($post->id); ?>');
            ...
            <dialog id="youtube-modal-<?php echo e($post->id); ?>" class="fixed inset-0 max-h-screen max-w-none z-50 rounded-lg shadow-2xl backdrop:bg-black backdrop:bg-opacity-80 p-0">
                <div class="w-full h-full flex flex-col bg-black">
                    
                    <div class="flex items-center justify-between p-4 border-b border-slate-700">
                        <h2 class="text-lg font-semibold text-white truncate flex-1"><?php echo e($post->title); ?></h2>
                        <button 
                            onclick="document.getElementById('youtube-modal-<?php echo e($post->id); ?>').close()"
                ...
                <script>
                    document.getElementById('youtube-modal-<?php echo e($post->id); ?>')?.addEventListener('keydown', (e) => {
                        if (e.key === 'Escape') {
                            document.getElementById('youtube-modal-<?php echo e($post->id); ?>').close();
                        }
                    });
                </script>
            </dialog>
    ...
    <?php elseif($isExternal && ($mediaTypeValue === 'article' || $mediaTypeValue === 'external_link')): ?>
        <?php
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($imageUrl): ?>
            ...
            <div class="absolute inset-0 w-full h-full min-h-[224px] bg-gradient-to-br from-emerald-50 to-slate-100 flex items-center justify-center p-8 hidden" id="fallback-<?php echo e($post->id); ?>">
                <div class="text-center max-w-2xl">
                    ...
                    <h1 class="text-lg font-bold text-slate-900 mb-2 line-clamp-2"><?php echo e($post->title); ?></h1>
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!empty($data['description'])): ?>
                        <p class="text-xs text-slate-600 mb-3 line-clamp-2"><?php echo e($data['description']); ?></p>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                </div>
            </div>
            
            <script>
                document.querySelectorAll('img[src="<?php echo e($imageUrl); ?>"]').forEach(img => {
                    img.addEventListener('error', function() {
                        this.style.display = 'none';
                        const fallback = document.getElementById('fallback-<?php echo e($post->id); ?>');
                        if (fallback) fallback.classList.remove('hidden');
                    });
                });
            </script>
    ...
    <?php else: ?>
        ...
        <div class="w-full h-full min-h-[224px] bg-gradient-to-br from-indigo-50 to-slate-100 flex items-center justify-center p-8">
            <div class="text-center max-w-xl">
                ...
                <p class="text-xs text-slate-500 mt-1"><?php echo e(strtoupper(str_replace('_', ' ', $mediaTypeValue))); ?></p>
            </div>
        </div>
