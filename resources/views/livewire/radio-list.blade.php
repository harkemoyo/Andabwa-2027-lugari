        ...
        <div class="bg-white rounded-2xl shadow-sm border  border-gray-100 overflow-hidden group">
            <div class="relative h-48">
                ...
                <div class="ustify-center items-center">
                    <img src="{{ $radio->full_cover_image_path }}" alt="" class="w-20 h-20 rounded-full object-contain object-center pointer-events-none">
                </div>
                @if($radio->type === 'live')
            ...
            <div class="p-6">
                <h3 class="text-xl font-bold text-gray-900 group-hover:text-indigo-600 transition">{{ $radio->title }}</h3>
                <p class="text-gray-500 text-sm mt-2 line-clamp-2">{{ $radio->description }}</p>
                ...
                <div class="mt-6 flex items-center justify-between">
                    ...
                    <button class="flex items-center gap-2 text-indigo-600 font-semibold text-sm">
                        ...
                        Listen Now ({{ $radio->duration_minutes }}m)
                    </button>
                    @else
                    <a href="{{ $radio->live_url }}" target="_blank" class="bg-gray-900 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-800">
                        Join Stream
                    </a>
                    @endif
                </div>
