...
$featuredMediaIsVideo = $featuredMedia && str_contains($featuredMedia->mime_type ?? '', 'video');
...
<div class="relative w-full aspect-video bg-slate-100 rounded-md overflow-hidden group isolate">
    ...
    $isVideo = $media && str_contains($media->mime_type ?? '', 'video');
    @endphp
    ...
    <video
        controls
        preload="metadata"
        poster="{{ $media && $media->hasGeneratedConversion('preview') ? $media->getUrl('preview') : '' }}"
    ...
    <img src="{{ $mediaUrl }}" alt="{{ $fallbackTitle }}" class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 pointer-events-none" loading="lazy">
    @endif    
    ...
    @php
    $imageUrl = $data['image'] ?? $post->youtube_thumbnail_url ?? null;
    $embedUrl = $data['embed_url'] ?? $post->youtube_embed_url ?? null;
    @endphp
    ...
    <div class="absolute inset-0 bg-gradient-to-br from-red-50 to-slate-100 flex items-center justify-center p-6 z-0 text-center">
        <div>
            <svg class="w-12 h-12 mx-auto text-red-400 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
            </svg>
    ...
    <div class="absolute inset-0 bg-gradient-to-br from-emerald-50 to-slate-100 hidden items-center justify-center p-6 z-0">
        <div class="text-center max-w-sm">
            ...
            <p class="text-sm font-semibold text-slate-900 line-clamp-2">{{ $fallbackTitle }}</p>
        </div>
    </div>
    @else
    {{-- No Image Available --}}
    <div class="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-emerald-50 to-slate-100 p-6 text-center">
        <div class="max-w-sm">
            ...
            <p class="text-sm font-semibold text-slate-800 line-clamp-2">{{ $fallbackTitle }}</p>
    ...
    <div class="absolute inset-0 flex items-center justify-center bg-slate-50 text-slate-400">
        <div class="text-center p-6">
            ...
            <p class="text-sm font-semibold text-slate-600 line-clamp-2">{{ $fallbackTitle }}</p>
            <span class="text-xs font-medium uppercase tracking-wider text-slate-400">No Media</span>
        </div>
    </div>
    @endif
    @endif
